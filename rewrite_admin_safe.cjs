const fs = require('fs');
const filePath = 'd:/Xgroup/Software/Indisutex/src/pages/Admin.tsx';
let content = fs.readFileSync(filePath, 'utf8');

const startMarker = `{/* 99 Envíos integration */}`;
                  
const startIndex = content.indexOf(startMarker);
const emptyStateIndex = content.indexOf('<div className="empty-state">', startIndex);

if (startIndex !== -1 && emptyStateIndex !== -1) {
    const before = content.substring(0, startIndex);
    const realAfterIndex = content.lastIndexOf(') : (', emptyStateIndex);

    const afterBlock = content.substring(realAfterIndex);
...
`;
    ...
    fs.writeFileSync(filePath, before + newBlock + afterBlock);
    console.log("Success");
} else {
    console.log("Failed to find markers", startIndex, emptyStateIndex);
}
